<template>
    <a-form :form="form" >
        <a-form-item label="导航标题" v-bind="formItemLayout">
            <a-input  placeholder="请输入标题名" v-model="nav_title"/>
        </a-form-item>
        ...
        <a-form-item   v-bind="formItemLayout" label="链接类型" has-feedback>
            <a-select defaultValue="请选择..." style="width: 100%" @change="handleProvinceChange" v-model="link_type">
        ...
        <a-form-item v-bind="formItemLayout" label="链接目标" has-feedback v-if="link[link_type]">
            <a-select  style="width: 100%" v-model="link_target">
                <a-select-option  v-for="(target,index) in link[link_type]" :value="target.id" :key="index">
